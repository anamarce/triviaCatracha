			...
			GUILayout.BeginHorizontal();
				...
				if ((_selectedCategoryIdx == UNUSED_ASSETS_IDX && _buildInfo.HasUnusedAssets))
				{
					...
					if (GUILayout.Button(pagePrevLabel) && (batchNumber - 1 >= 1))
					{
						...
						BuildReportTool.ReportManager.MoveUnusedAssetsBatchToPrev(_buildInfo);
					}
					...
					{
						// move to next batch
						BuildReportTool.ReportManager.MoveUnusedAssetsBatchToNext(_buildInfo);
					}
				...
				string paginateLabel = "Viewing page " + offsetNonZeroBased + " - " + len + " of " + assetListToUse.Length;
