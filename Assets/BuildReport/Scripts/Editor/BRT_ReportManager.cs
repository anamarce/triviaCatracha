...
public partial class ReportManager
{
	...
	static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject)
	{
		...
		if (BRT_BuildReportWindow.IsOpen || BuildReportTool.Options.ShowAfterBuild)
		{
	...
	public static void MoveUnusedAssetsBatchToNext(BuildInfo buildInfo)
	{
		buildInfo.MoveUnusedAssetsBatchNumToNext();
		RefreshUnusedAssetsBatch(buildInfo);
	}

	public static void MoveUnusedAssetsBatchToPrev(BuildInfo buildInfo)
	{
	...
	static void RefreshUnusedAssetsBatch(BuildInfo buildInfo)
	{
		if (buildInfo.UnusedAssetsIncludedInCreation)
		{
			...
			if (allUnused != null && allUnused.Length > 0)
			{
				...
				if (allUsed.Count != buildInfo.UsedAssets.AllCount)
				{
					...
					BuildReportTool.SizePart[][] newPerCategoryUsed = SegregateAssetSizesPerCategory(newAllUsedArray, buildInfo.FileFilters);
					buildInfo.UsedAssets = new AssetList();
					buildInfo.UsedAssets.Init(newAllUsedArray, newPerCategoryUsed, buildInfo.FileFilters);
				}
			}
			else
			{
...
*/
	...
	static BuildPlatform GetBuildPlatformFromString(string gotBuildType, BuildTarget buildTarget)
	{
